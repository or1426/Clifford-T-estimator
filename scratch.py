...
if __name__ == "__sf__":
    ...
    for  i, circ in enumerate(util.random_clifford_circuits_with_bounded_T(qubits, depth, circs, t)):
        ...
        with open("t=80-sim.txt", "w") as f:
            print(qubits, t, comp_val, comp_time, file = f)
...
if __name__ == "__qk_test__":
    ...
    for qubits in qubitss:
        for  i, circ in enumerate(util.random_clifford_circuits_with_bounded_T(qubits, depth, circs, t)):
            ...
            #with open("no_qk_calculate_comparison_{}.txt".format(qubits), "a") as f:
            #    #print(qubits, qk_val, comp_val, qk_time, comp_time, file=f)
            #    print(qubits, comp_val, comp_time, file=f)
            
            

if __name__ == "__t-minus-r-hist__":
    qubits = 50
    t = 40
    depth = 100000
    circs = 1000
...
if __name__ == "__main__":
    ...
    for  i, circ in enumerate(util.random_clifford_circuits_with_bounded_T(qubits, depth, circs, t)):
        ...
        for eps in epss:
            ...
            t, r, log_v, pyChState, pyAGState = cPSCS.compress_algorithm(qubits, measured_qubits, np.copy(gateArray), np.copy(controlArray), np.copy(targetArray), np.copy(aArray))
            beta = np.log2(4. - 2.*np.sqrt(2.));
            m = np.power(2, beta*t/2)
            p = estimate.optimize(eps, 0.001, t,measured_qubits, r, log_v, m, pyChState, pyAGState)
            print("optimise: ", p)
